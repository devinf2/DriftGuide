-- Active fishing duration (ms), excluding paused intervals — matches client pause/resume tracking.
alter table public.trips
  add column if not exists active_fishing_ms bigint;

comment on column public.trips.active_fishing_ms is 'Milliseconds of active trip time excluding pauses; null for legacy rows.';

-- Extend batch import RPC to accept optional active_fishing_ms on trip JSON.

-- Batch import: single-transaction RPC for importing multiple trips with their
-- events, conditions snapshots, catches, and photo rows.
-- Accepts JSONB arrays and performs all inserts inside one transaction.
-- The existing sync_community_catch trigger on catches fires automatically.
--
-- conditions_snapshot IDs are pre-generated by the caller and included in both
-- p_conditions (as "id") and p_catches (as "conditions_snapshot_id"), so no
-- loop or RETURNING correlation is needed.
...
begin
  ...
  insert into trips (
    ...
    session_type, rating, user_reported_clarity, imported
  )
  select
    (r->>'id')::uuid,
    (r->>'user_id')::uuid,
  ...
  -- 3. Insert conditions_snapshots (IDs pre-generated by caller)
  ...
  -- 4. Upsert all catches (conditions_snapshot_id already set by caller)
  insert into catches (

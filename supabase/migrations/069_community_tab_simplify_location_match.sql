-- Community tab: exact location_id only, no location tree helper.
-- Tab visible iff ≥1 completed trip in last 30d with rating + public photo visibility.
-- List = all completed trips at that location with rating + public visibility (newest first).
-- After apply: notify pgrst, 'reload schema';

drop function if exists public.location_related_ids(uuid);
